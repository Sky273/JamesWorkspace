...
function normalizeSuggestionItems(value: unknown): string[] {
  ...
  if (Array.isArray(value)) {
    return [...new Set(value.flatMap((item) => normalizeSuggestionItems(item)).filter(Boolean))];
  }
  if (typeof value === 'object') {
    ...
    return [...new Set(Object.values(record).flatMap((item) => normalizeSuggestionItems(item)).filter(Boolean))];
  }
  return [];
}
