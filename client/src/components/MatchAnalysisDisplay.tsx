...
const MatchAnalysisDisplay = ({ analysis, onContinue, onCancel, hideActions = false }: MatchAnalysisDisplayProps): JSX.Element | null => {
  ...
  const sectionTitles: Record<string, string> = {
    executiveSummary: t('resume.analysis.sections.executiveBrief'),
    skills: t('resume.analysis.sections.skillsKeywords'),
    experience: t('resume.analysis.sections.experience'),
    education: t('resume.analysis.sections.education'),
    atsOptimization: t('resume.analysis.sections.atsOptimization')
  };
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className={`p-6 rounded-lg ${getScoreBgColor(matchScore)}`}>
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">{t('adaptations.matchAnalysis.matchScore')}</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">{t('adaptations.matchAnalysis.subtitle')}</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {analysis.strengths && analysis.strengths.length > 0 && (
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4"><CheckCircleIcon className="w-6 h-6 text-green-500" /><h4 className="font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.strengths')}</h4></div>
        ...
        {analysis.gaps && analysis.gaps.length > 0 && (
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4"><ExclamationTriangleIcon className="w-6 h-6 text-yellow-500" /><h4 className="font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.gaps')}</h4></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {analysis.keywordMatches && analysis.keywordMatches.length > 0 && (
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4"><ChartBarIcon className="w-6 h-6 text-blue-500" /><h4 className="font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.keywordMatches')}</h4></div>
        ...
        {analysis.missingKeywords && analysis.missingKeywords.length > 0 && (
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4"><SparklesIcon className="w-6 h-6 text-purple-500" /><h4 className="font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.missingKeywords')}</h4></div>
      ...
      {analysis.recommendations && (Array.isArray(analysis.recommendations) ? analysis.recommendations.length > 0 : Object.keys(analysis.recommendations).length > 0) && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
          <h4 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">{t('adaptations.matchAnalysis.recommendations')}</h4>
      ...
      {!hideActions && (
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          <button onClick={onCancel} className="px-6 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">{t('common.cancel')}</button>
          <button onClick={onContinue} className="px-6 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium transition-colors flex items-center gap-2"><SparklesIcon className="w-5 h-5" />{t('adaptations.matchAnalysis.generateAdaptedCv')}</button>
        </div>
      )}
    </motion.div>
  );
