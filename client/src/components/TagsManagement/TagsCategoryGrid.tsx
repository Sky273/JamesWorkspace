...
export default function TagsCategoryGrid({
  ...
  const categoryEntries = activeTab === 'esco' ? Object.entries(filteredEscoTags) : Object.entries(source);
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      {activeTab !== 'esco' &&
        categoryEntries.map(([category, tagList], categoryIndex) => {
              ...
              <div className={`flex items-center gap-3 border-b p-5 ${borderClass}`}>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    {tagList?.length || 0} {t('tags.tagsCount')}
                  </p>
                </div>
                {isCleanedTab && (
