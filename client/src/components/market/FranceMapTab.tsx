...
export default function FranceMapTab({ className = '' }: { className?: string }) {
  ...
  const formatValue = useCallback(
    (value: number | string | undefined | null) => {
      ...
      if (dataSource === 'all' || dataSource === 'offres') return numericValue > 999 ? `${Math.round(numericValue / 1000)}k` : Math.round(numericValue).toString();
      if (dataSource === 'salaire') return `${Math.round(numericValue / 1000)}kEUR`;
      if (dataSource === 'tension') return numericValue.toFixed(1);
  ...
  const statsCards = useMemo(
    () => [
      ...
      {
        value: String(currentRegionData.length),
        label: t('marketRadar.map.regionsCovered'),
        color: 'text-blue-600 dark:text-blue-400',
      },
      {
        value: topRegion?.name || '-',
        label: t('marketRadar.map.topRegion'),
        color: 'text-green-600 dark:text-green-400',
      },
      {
        value: String(uniqueMetiersCount),
        label: t('marketRadar.map.itJobs'),
        ...
        {dataSource === 'all' ? (
          <div className="flex items-center gap-4 flex-wrap">
            {DATA_SOURCE_OPTIONS.filter((option) => option.value !== 'all' && option.value !== 'salaire').map((option) => {
              ...
              return (
                <button key={option.value} onClick={() => setDataSource(option.value)} className="flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer">
