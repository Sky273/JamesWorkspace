  ...
  if (!matchAnalysis) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 dark:text-gray-400">{t('adaptations.matchAnalysis.noAnalysis')}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4"><CheckCircleIcon className="w-6 h-6 text-green-600 dark:text-green-400" /><h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.strengths')}</h3></div>
          {strengths.length > 0 ? (
          ...
          ) : (<p className="text-sm text-gray-500 dark:text-gray-400">{t('adaptations.matchAnalysis.noStrengths')}</p>)}
        </div>

        <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4"><ExclamationTriangleIcon className="w-6 h-6 text-yellow-600 dark:text-yellow-400" /><h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.gaps')}</h3></div>
          {gaps.length > 0 ? (
          ...
          ) : (<p className="text-sm text-gray-500 dark:text-gray-400">{t('adaptations.matchAnalysis.noGaps')}</p>)}
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4"><ChartBarIcon className="w-6 h-6 text-blue-600 dark:text-blue-400" /><h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.keywordMatches')}</h3></div>
          ...
          ) : (<p className="text-sm text-gray-500 dark:text-gray-400">{t('adaptations.matchAnalysis.noKeywordMatches')}</p>)}
        </div>

        <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4"><ChartBarIcon className="w-6 h-6 text-purple-600 dark:text-purple-400" /><h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('adaptations.matchAnalysis.missingKeywords')}</h3></div>
          ...
          ) : (<p className="text-sm text-gray-500 dark:text-gray-400">{t('adaptations.matchAnalysis.noMissingKeywords')}</p>)}
        </div>
      </div>
    </div>
  );
