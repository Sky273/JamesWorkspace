...
export default function ResponsivePageTabs<T extends string>({
  ...
  return (
    <div className={`flex flex-col gap-2 ${label ? 'mb-5' : ''}`.trim()}>
      {label ? <span className="cv-kicker">{label}</span> : null}
      <div
        className="segmented-control grid gap-1 rounded-[13px] p-1"
        style={{
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              title={option.label}
              className={`segmented-control__item inline-flex min-h-10 w-full min-w-0 items-center gap-1.5 rounded-[9px] px-2 py-2 text-left text-sm font-semibold sm:px-2.5 ${
                isActive ? 'segmented-control__item--active' : ''
              }`}
            >
              {Icon ? <Icon className="h-4 w-4 shrink-0" /> : null}
              <span className="min-w-0 flex-1 whitespace-normal break-words text-[0.86rem] leading-snug">
                {option.label}
              </span>
            </button>
          );
