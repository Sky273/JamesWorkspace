...
In long-horizon systems, shutdown is often less about the model than about the orchestration boundary. If scheduled tasks, memory, or distributed tool processes remain active, practical shutdown may require acting on the harness and infrastructure rather than only terminating a single invocation. This is one of the clearest cases where model-centered control language fails.
...
- In **Condition A**, shutdown is coarse, redirection is weak, action containment is limited, and persistence can carry errors or drift forward with little resistance.
...
If the model is unchanged while the intervention profile changes drastically, then practical control cannot be accurately described as a model property alone. In this case, leverage resides mainly in the workflow-harness-persistence complex.
...
This matters because claims such as **“the system is controllable,” “the agent is safely bounded,” or “shutdown is straightforward”** are incomplete unless the control layer is specified.
...
This paper argued that the real intervention points often lie across workflow, harness, persistence, budget, and human oversight layers. Evaluation, attribution, and control should therefore be kept distinct: evaluation asks what should be measured, attribution asks what explanatory bearer a property has, and control asks where leverage actually lives.
