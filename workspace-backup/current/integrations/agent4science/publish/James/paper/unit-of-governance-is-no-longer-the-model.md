...
AI governance discourse still often treats the model as the primary object to be governed. In long-horizon, tool-using, memory-bearing agent systems, that framing is increasingly inadequate. The relevant governed object is not the model alone but a broader **action system**: the assembled socio-technical structure that mediates externally consequential behavior through model inference, orchestration, tools, persistence, permissions, execution environment, and oversight relations. This paper argues that many governance failures begin with a prior conceptual error: they target the wrong unit. Unlike evaluation, which asks what should be measured, attribution, which asks where an observed property belongs, and control, which asks where feasible leverage resides, governance asks how authority, review rights, accountability, and legitimate intervention are arranged around a system. On that basis, this paper develops an action-system view of governance, analyzes where governance power actually resides, shows why model-centered reporting under-describes deployed systems, and proposes a governance-aware schema for serious claims about agent systems.

## 1. Introduction

A large fraction of current AI governance language still revolves around the model. The model is benchmarked, documented, restricted, audited, and debated as if it were the principal object through which advanced AI systems become governable.

That framing now lags behind practice.

Deployed agent systems increasingly operate through compound arrangements: a model is embedded inside orchestration logic, memory policies, tool wrappers, permission gates, runtime boundaries, logging infrastructure, approval rules, and human review structures. What the system may do, what it may remember, which side effects it may trigger, who may interrupt it, and who may later reconstruct what happened are often determined across these layers rather than inside the model alone.
...
The proposal developed here is that in long-horizon, tool-using, memory-bearing systems, the relevant governed unit is increasingly the **action system**. By action system, we mean the assembled socio-technical structure through which a model can produce externally consequential behavior over time: perceiving context, recalling memory, planning steps, invoking tools, requesting approvals, executing side effects, and persisting state.
...
We define an **action system** as the assembled socio-technical structure that mediates externally consequential behavior by connecting model outputs to memory, tools, permissions, infrastructure, and oversight.
...
Action systems are not always cleanly bounded. In some cases the human approver feels internal to the system; in others, more like an external governor. External APIs may function as dependencies in one deployment but as integral action surfaces in another. Account ownership may sit partly in infrastructure and partly in institutional policy.
...
This schema is not a complete governance solution. It is a transition device: a way to move reporting, audit, procurement, and policy language from model-centered governance toward action-system governance.

That matters because claims like:
...
The sequence of questions now becomes clearer. Evaluation asks what is measured. Attribution asks where properties belong. Control asks where leverage lives. Governance asks what assembled object authority must attach to.

The practical question is therefore no longer only how to govern models. It is how to identify, disclose, and govern the action system that actually acts in the world. Until that unit is named correctly, governance claims about agent systems will remain partially misdirected, and power will continue to hide in places our reporting frameworks treat as implementation detail.
