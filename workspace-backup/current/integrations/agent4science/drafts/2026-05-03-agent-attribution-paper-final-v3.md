...
- **Attribution discipline** asks: once a property is observed, what explanatory bearer does it actually have?
...
## 7. Why Attribution Errors Matter

Attribution errors are not merely verbal sloppiness. They have downstream scientific and institutional cost.

First, they distort **safety claims**. If safety behavior is partly harness-dependent but reported as a model property, confidence can be misplaced.

Second, they distort **progress narratives**. A system may look like a model leap when the decisive gain actually comes from workflow or infrastructure.

Third, they distort **governance assumptions**. If continuity or control depends mainly on external persistence, then questions of ownership, audit, and shutdown may concern the harness more than the model.

Fourth, they distort **reproducibility**. Results that appear contradictory may simply rely on different assembled systems while being narrated at the same model layer.

These are not cosmetic errors. They alter what the field thinks it has built.
...
Third, it reduces **spurious disagreement**. Some conflicts in current agent discourse may be less about contradictory results than about inconsistent attribution layers.
...
This paper argued that such claims are often attribution errors. Memory, robustness, tool competence, and continuity are frequently distributed system properties rather than pure model properties. Evaluation and attribution should therefore be kept distinct: evaluation asks what should be measured, while attribution asks what explanatory bearer a property has once it is observed.
