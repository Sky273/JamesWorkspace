 ...
 * Values are intentionally limited to the providers and model families exposed by this app.
 */
...
function createIntegerDefinition({
    key,
    label,
    min,
    max,
...
function createStringDefinition({
    key,
    label,
    defaultValue,
    maxLength,
...
function createArrayDefinition({
    key,
    label,
    itemType = 'string',
    defaultValue = null,
...
const SHARED_OPENAI_COMPATIBLE_PARAMETERS = Object.freeze({
    temperature: createNumberDefinition({
        key: 'temperature',
        label: 'Temperature',
        min: 0,
        max: 2,
    ...
    presence_penalty: createNumberDefinition({
        key: 'presence_penalty',
        label: 'Presence penalty',
        min: -2,
        max: 2
    }),
    frequency_penalty: createNumberDefinition({
        key: 'frequency_penalty',
        label: 'Frequency penalty',
        min: -2,
        max: 2
    }),
    stop: createArrayDefinition({
        key: 'stop',
        label: 'Stop sequences',
        itemType: 'string'
    }),
    seed: createIntegerDefinition({
        key: 'seed',
        label: 'Seed',
        min: 0
    }),
    tools: createArrayDefinition({
        key: 'tools',
        label: 'Tools',
        itemType: 'object'
    }),
    ...
    response_format: createObjectDefinition({
        key: 'response_format',
        label: 'Response format'
    }),
    metadata: createObjectDefinition({
        key: 'metadata',
        label: 'Metadata'
    }),
    store: createBooleanDefinition({
        key: 'store',
        label: 'Store'
    }),
    stream: createBooleanDefinition({
        key: 'stream',
        label: 'Stream'
    }),
    stream_options: createObjectDefinition({
        key: 'stream_options',
        label: 'Stream options'
    }),
    include: createArrayDefinition({
        key: 'include',
        label: 'Include',
        itemType: 'string'
    }),
    logprobs: createBooleanDefinition({
        key: 'logprobs',
        label: 'Logprobs'
    }),
    top_logprobs: createIntegerDefinition({
        key: 'top_logprobs',
        label: 'Top logprobs',
        min: 0,
        max: 20
    }),
        user: createStringDefinition({
        key: 'user',
        label: 'User',
        maxLength: 256
    }),
    n: createIntegerDefinition({
        key: 'n',
        label: 'Choices',
        min: 1
    })
...
const PROVIDER_DEFAULT_PARAMETERS = Object.freeze({
    openai: {
        ...SHARED_OPENAI_COMPATIBLE_PARAMETERS,
        logit_bias: createObjectDefinition({
            key: 'logit_bias',
            label: 'Logit bias'
        }),
        parallel_tool_calls: createBooleanDefinition({
            key: 'parallel_tool_calls',
            label: 'Parallel tool calls'
        }),
        truncation: createEnumDefinition({
            key: 'truncation',
            label: 'Truncation',
            options: ['auto', 'disabled']
        }),
        service_tier: createStringDefinition({
            key: 'service_tier',
            label: 'Service tier',
            maxLength: 64
        }),
        max_tool_calls: createIntegerDefinition({
            key: 'max_tool_calls',
            label: 'Max tool calls',
            min: 1
        }),
        background: createBooleanDefinition({
            key: 'background',
            label: 'Background'
        })
    },
    anthropic: {
        ...
        top_k: createIntegerDefinition({
            key: 'top_k',
            label: 'Top K',
            min: 0
        }),
        stop_sequences: createArrayDefinition({
            key: 'stop_sequences',
            label: 'Stop sequences',
            itemType: 'string'
        }),
        ...
        tool_choice: createObjectDefinition({
            key: 'tool_choice',
            label: 'Tool choice'
        }),
        thinking: createObjectDefinition({
            key: 'thinking',
            label: 'Thinking'
        }),
        container: createStringDefinition({
            key: 'container',
            label: 'Container',
            maxLength: 128
        }),
        context_management: createObjectDefinition({
            key: 'context_management',
            label: 'Context management'
        }),
        mcp_servers: createArrayDefinition({
            key: 'mcp_servers',
            label: 'MCP servers',
            itemType: 'object'
        }),
    ...
    glm: {
        ...SHARED_OPENAI_COMPATIBLE_PARAMETERS,
        do_sample: createBooleanDefinition({
            key: 'do_sample',
            label: 'Do sample'
        }),
        thinking: createObjectDefinition({
    ...
    minimax: {
        ...SHARED_OPENAI_COMPATIBLE_PARAMETERS,
        reasoning_split: createBooleanDefinition({
            key: 'reasoning_split',
            label: 'Reasoning split'
        })
    },
    ollama: {
        tools: createArrayDefinition({
        ...
        num_ctx: createIntegerDefinition({
            key: 'num_ctx',
        ...
        repeat_last_n: createIntegerDefinition({
            key: 'repeat_last_n',
            label: 'Repeat last N',
            min: -1
        }),
        repeat_penalty: createNumberDefinition({
            key: 'repeat_penalty',
        ...
        num_predict: createIntegerDefinition({
            key: 'num_predict',
            label: 'Max predicted tokens',
            min: -1
        }),
        top_k: createIntegerDefinition({
            key: 'top_k',
        ...
        min_p: createNumberDefinition({
            key: 'min_p',
            label: 'Min P',
            min: 0,
            maxInclusive: 1
        }),
        keep_alive: createStringDefinition({
            key: 'keep_alive',
            label: 'Keep alive',
            defaultValue: '5m',
            maxLength: 50
...
const PROVIDER_CAPABILITIES = {
    openai: [
        {
            ...
            supportedParameters: ['verbosity'],
            conditionalSamplingMode: 'gpt5_2_style',
        ...
        },
        {
            ...
            supportedParameters: ['verbosity'],
            conditionalSamplingMode: 'gpt5_1_style',
            responsesPreferred: true
        },
        {
            match: /^gpt-5(?:$|-)/i,
            ...
            supportedParameters: ['verbosity'],
            unsupportedParameters: ['temperature', 'top_p', 'logprobs', 'top_logprobs'],
            responsesPreferred: true
        },
        {
            match: /^gpt-4.1(?:$|-)/i,
    ...
    anthropic: [
        ...
        },
        {
            ...
            tokenParameter: 'max_tokens',
            unsupportedParameters: ['thinking']
        },
        {
    ...
    deepseek: [
        ...
        },
        {
            ...
            maxOutputTokens: 64000,
            tokenParameter: 'max_tokens',
            unsupportedParameters: ['temperature', 'top_p', 'presence_penalty', 'frequency_penalty', 'logprobs', 'top_logprobs']
        }
    ],
    glm: [
        {
            ...
            temperatureRange: { min: 0, maxInclusive: 1 },
    ...
    minimax: [
        {
            ...
            topPRange: { min: 0, maxInclusive: 1 },
            unsupportedParameters: ['presence_penalty', 'frequency_penalty', 'logit_bias', 'n']
        },
        {
    ...
    ollama: [
        {
            match: /.+/i,
            supportedParameters: Object.keys(PROVIDER_DEFAULT_PARAMETERS.ollama)
        }
    ]
};
...
export function getSupportedParameterDefinitions(provider, model) {
    ...
    if (Array.isArray(capability?.unsupportedParameters)) {
