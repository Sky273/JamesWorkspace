...
import {
    ...
    getNamedCacheStats,
    invalidateResumeGroupedViewCaches,
...
export function getStatsCacheStats() {
    const localSize = statsCache.size;
    return buildApplicationCacheMetrics({
        size: localSize,
        maxSize: MAX_STATS_CACHE_ENTRIES,
        ttlMs: STATS_CACHE_TTL,
        extra: {
            groupedViewEntries: 0
        }
    });
}

export async function getResumeGroupedViewCacheStats() {
    return getNamedCacheStats('resumeGroupedViews');
}

export async function invalidateGroupedResumeViewsCache(firmId = null) {
    const scopeKey = firmId ? buildGroupedViewScopeKey({ firmId }) : null;
    await invalidateResumeGroupedViewCaches(scopeKey);
}

export async function invalidateResumeViewCaches(firmId = null) {
    invalidateStatsCache(firmId);
    await invalidateGroupedResumeViewsCache(firmId);
}
