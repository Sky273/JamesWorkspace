...
function requireTemplateManager(req, res) {
...
function ensureTemplateOwnership(req, res, template, firmId, { allowSystemTemplate = true, systemTemplateError = 'Cannot modify system template' } = {}) {
    ...
    if (!superAdmin && template.firm_id !== firmId) {
...
router.get('/', authenticateToken, createEmailTemplatesRouteHandler('Error fetching email templates', 'Failed to fetch email templates', async (req, res) => {
        if (!requireTemplateManager(req, res)) {
            return;
        }
...
router.delete('/:id', authenticateToken, validateParams('id'), createEmailTemplatesRouteHandler('Error deleting email template', 'Failed to delete email template', async (req, res) => {
        ...
        await emailTemplatesService.deleteTemplate(id, { isAdmin: isUserAdmin(req) });
...
router.post('/:id/duplicate', authenticateToken, validateParams('id'), createEmailTemplatesRouteHandler('Error duplicating email template', 'Failed to duplicate email template', async (req, res) => {
        ...
        const { id } = req.params;
        const actorFirmId = await getRequestFirmId(req);
        const userId = getAuthenticatedUserId(req.user);
        const requestedFirmId = typeof req.body?.firmId === 'string' ? req.body.firmId.trim() : '';
        
        if (!actorFirmId && !isUserAdmin(req)) {
            return res.status(403).json({ error: 'No firm association' });
        }

        if (!actorFirmId && isUserAdmin(req) && !requestedFirmId) {
            return res.status(400).json({ error: 'Target firm is required' });
        }

        const targetFirmId = isUserAdmin(req) ? requestedFirmId || actorFirmId : actorFirmId;

        if (!targetFirmId) {
            return res.status(400).json({ error: 'Firm ID not found for user. Please contact administrator.' });
        }

        const targetFirm = await emailTemplatesService.getFirmById(targetFirmId);
        if (!targetFirm) {
        ...
        const template = await emailTemplatesService.duplicateTemplate(id, targetFirmId, userId);
